...
#define apc_test_alloc(X) do {if ((void *)(X) == NULL) fprintf(stderr, "Out of memory in %s, (%s, line %d).\n", __FUNCTION__, __FILE__, __LINE__); } while (0)

void init(apc_problem_t* apc, int *u, int *v, int *fb, int *p, int *m_p);
void path(apc_problem_t* p, int *u, int *v, const int *fb, int *rc, int *pi, int *lr, int *uc, int ii, int *jj_p);
void incr(apc_problem_t* p, int *fb, const int *rc, int j);

int apc_init(apc_problem_t* p, int n, int** a, int z_p, int s) {
    p ->num_cols = n;
    p ->number_solution = s;
    p->cost_optmal_assignment = z_p;

    p ->cost = (int**) calloc(n, sizeof (int*));
    apc_test_alloc(p->cost);

    p->colunm_assignment = (int*) calloc(n, sizeof (int));
    apc_test_alloc(p->colunm_assignment);

    for (int i = 0; i < p->num_cols; i++) {
        p->cost[i] = (int*) calloc(n, sizeof (int));
        apc_test_alloc(p->cost[i]);
        for (int j = 0; j < p->num_cols; j++) {
            p->cost[i][j] = (i < n && j < n) ? a[i][j] : 0;
...
void apc_free(apc_problem_t* p) {
    int i;
    for (i = 0; i < p->num_cols; i++) {
        free(p->cost[i]);
    }
    free(p->cost);
    free(p->colunm_assignment);

    p->cost = NULL;
    p->colunm_assignment = NULL;
    free(p);
}
...
int apc(apc_problem_t* p) {
     ...
     */
    u = (int*) malloc(p ->num_cols * sizeof (int));
    if (u == NULL) {
        return -1;
    }
    v = (int*) malloc(p ->num_cols * sizeof (int));
    if (v == NULL) {
        free(u);
        return -1;
    }
    fb = (int*) malloc(p ->num_cols * sizeof (int));
    if (fb == NULL) {
        free(u);
        free(v);
        return -1;
    }
    rc = (int*) malloc(p ->num_cols * sizeof (int));
    if (rc == NULL) {
        free(u);
        free(v);
        free(fb);
    ...
    pi = (int*) malloc(p ->num_cols * sizeof (int));
    if (pi == NULL) {
        free(u);
        free(v);
        free(fb);
    ...
    lr = (int*) malloc(p ->num_cols * sizeof (int));
    if (lr == NULL) {
        free(u);
        free(v);
        free(fb);
    ...
    uc = (int*) malloc(p ->num_cols * sizeof (int));
    if (uc == NULL) {
        free(u);
        free(v);
        free(fb);
    ...
    if (m != p ->num_cols) {
        for (i = 0; i < p ->num_cols; i++) {
            if (p->colunm_assignment[i] < 0) {
                //DETERMINATION OF AN AUGMENTING PATH STARTING FROM ROW  I .
                path(p, u, v, fb, rc, pi, lr, uc, i, &j);
    ...
    // Parte do Cristofer ---*
    // Recuces the cost matrix
    for (i = 0; i < p ->num_cols; i++) {
        for (j = 0; j < p ->num_cols; j++)
            p->cost[i][j] = p->cost[i][j] - u[i] - v[j];
    ...
    (p->cost_optmal_assignment) = 0;
    for (k = 0; k < p ->num_cols; k++) {
        (p->cost_optmal_assignment) += (u[k] + v[k]);
    }
...
void incr(apc_problem_t* p, int *fb, const int *rc, int j) {
    ...
    do {
        i = rc[j];
        fb[j] = i;
        jj = p->colunm_assignment[i];
        p->colunm_assignment[i] = j;
        j = jj;
    } while (j >= 0);
...
void init(apc_problem_t* apc, int *u, int *v, int *fb, int *p, int *m_p) {
    ...
    int i, j, jmin, k, kk, r = 0, min, m, ia, skip;


    //PHASE 1 .

    m = 0;
    for (k = 0; k < apc-> num_cols; k++) {
        apc->colunm_assignment[k] = fb[k] = -1;
    }

    //SCANNING OF THE COLUMNS ( INITIALIZATION OF  V(J) ).
    for (j = 0; j < apc-> num_cols; j++) //40
    {
        min = INF;
        for (i = 0; i < apc-> num_cols; i++) //30
        {
            ia = apc->cost[i][j];
            if (ia < min || (ia == min && apc->colunm_assignment[i] < 0)) {
                min = ia;
                r = i;
            }
        }
        v[j] = min;
        if (apc->colunm_assignment[r] < 0) {
            //ASSIGNMENT OF COLUMN  J  TO ROW  R .
            m++;
            fb[j] = r;
            apc->colunm_assignment[r] = j;
            u[r] = 0;
            p[r] = j + 1;
        }
    }
    ...
    for (i = 0; i < apc-> num_cols; i++) // DO 110
    {
        if (apc->colunm_assignment[i] < 0) {
            min = INF;
            for (k = 0; k < apc-> num_cols; k++) //60
            {
                ia = apc->cost[i][k] - v[k];
            ...
            if (fb[j] >= 0) {
                skip = 0;
                for (j = jmin; j < apc-> num_cols && !skip; j++) //80
                {
                    if (apc->cost[i][j] - v[j] <= min) {
                        r = fb[j];
                        kk = p[r];
                        if (kk < apc-> num_cols) {
                            for (k = kk; k < apc-> num_cols && !skip; k++) //70
                            {
                                if (fb[k] < 0 && apc->cost[r][k] - u[r] - v[k] == 0) {
                                    //REASSIGNMENT OF ROW  R  AND COLUMN  K .
                                    apc->colunm_assignment[r] = k;
                                    fb[k] = r;
                                    p[r] = k + 1;
                                    //ASSIGNMENT OF COLUMN  J  TO ROW  I .
                                    m++;
                                    apc->colunm_assignment[i] = j;
                                    fb[j] = i;
                                    p[i] = j + 1;
